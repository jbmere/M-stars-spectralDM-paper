...
the quality of features from the two approaches (full spectrum and features
oriented) are equivalent in
...
% Add from JOM 
\textbf{
It must be stated that \cite{cesetti} have also errors. Therefore,
the goal is not to qualify the refression models proposed against such 
estimations, but to compare the behaviour of the selected features.
}
% End of comment
...
Finally, the same ML models are trained to infer the
